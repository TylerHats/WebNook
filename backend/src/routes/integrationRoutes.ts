...
// ----------------------------------------------------
// Spotify User Account OAuth Connect Routes
// ----------------------------------------------------
router.get('/spotify/login', async (req: Request, res: Response) => {
  try {
    const idRow = await queryOne<any>('SELECT value FROM system_settings WHERE key = "spotify_client_id"');
    const clientId = idRow?.value;
    ...
    if (!clientId) {
      return res.status(400).send(`
        <div style="font-family: system-ui, sans-serif; background: #0f101d; color: #ffffff; padding: 3rem 1.5rem; text-align: center; min-height: 100vh;">
          <h2 style="color: #facc15; font-size: 1.6rem; margin-bottom: 0.5rem;">⚠️ Spotify Integration Not Configured</h2>
          <p style="opacity: 0.8; max-width: 500px; margin: 0 auto 1.5rem; line-height: 1.5;">
            An administrator must configure the <strong>Spotify Client ID</strong> and <strong>Client Secret</strong> in the Admin Integrations tab before users can connect Spotify accounts.
          </p>
          <a href="/customize" style="background: #6366f1; color: #fff; text-decoration: none; padding: 0.6rem 1.2rem; borderRadius: 8px; font-weight: 700;">
            ← Return to Nook Customizer
          </a>
        </div>
      `);
    }
    ...
    const scope = encodeURIComponent('user-top-read user-read-recently-played');
    ...
    return res.status(500).send('Spotify login error');
  }
});
...
router.get('/spotify/callback', async (req: Request, res: Response) => {
  try {
    ...
    if (!code) return res.redirect('/customize?error=no_spotify_code');
    ...
    if (!clientId || !clientSecret) return res.redirect('/customize?error=spotify_credentials_missing');
    ...
    const tokenData = JSON.parse(tokenText);
    if (!tokenData.access_token) throw new Error('Failed to exchange Spotify token');

    return res.redirect('/customize?spotify_connected=1');
  } catch (err: any) {
    return res.redirect(`/customize?error=${encodeURIComponent(err.message)}`);
  }
});
